<template>
	<div v-if="isAdmin || isDeveloper" class="text-gray-300">
		<h2 class="text-white text-lg md:text-xl font-bold mb-4">
			Управление ролями
		</h2>
		<p class="text-[#D2EE5B] mb-4 text-sm md:text-base">
			Доступ разрешен (Ваша роль: {{ userData?.role || currentUser?.role }})
		</p>

		<div class="bg-[#2A2A2A] p-3 md:p-4 rounded-lg mb-4">
			<div
				class="flex flex-col sm:flex-row items-center space-y-3 sm:space-y-0 sm:space-x-4 mb-4"
			>
				<input
			...
			<div v-if="searchResult" class="bg-[#1A1A1A] p-3 md:p-4 rounded-lg mb-4">
				<h3 class="text-white font-semibold mb-2 text-sm md:text-base">
					Найден пользователь:
				</h3>
				<p class="text-sm md:text-base">
					<strong>Имя:</strong> {{ searchResult.name || searchResult.username }}
				</p>
				<p class="text-sm md:text-base">
					<strong>Текущая роль:</strong> {{ searchResult.role }}
				</p>

				<div
					class="flex flex-col sm:flex-row items-center space-y-3 sm:space-y-0 sm:space-x-4 mt-4"
				>
					<select
			...
			<div
				v-if="message"
				class="mt-4 p-3 rounded-lg text-sm md:text-base"
				:class="
	...
	<div v-else class="text-center py-6 md:py-8">
		...
		<p class="text-gray-400 mt-2 text-sm md:text-base">
			У вас недостаточно прав для управления ролями
		</p>
		<p class="text-gray-400 text-sm md:text-base">
			Ваша роль: {{ userData?.role || currentUser?.role || 'user' }}
		</p>
		<p class="text-gray-400 text-sm md:text-base">
			Требуется: admin или developer
		</p>
	</div>
</template>
